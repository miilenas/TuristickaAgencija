...
{
    public class CreatePutnikSO : SystemOperationBase
    {
        private readonly Putnik putnik;

        public Putnik Result { get; set; }

        public CreatePutnikSO(Putnik putnik)
        {
            this.putnik = putnik;
        }
        ...
        private void ValidatePutnik()
        {
            ...
            bool postojiPutnikSaIstimPasosem = broker.GetAll(new Putnik())
                .Cast<Putnik>()
                .Any(p => p.BrojPasosa.Equals(putnik.BrojPasosa, StringComparison.OrdinalIgnoreCase));
        ...
        {
            ValidatePutnik();
            Result = (Putnik)broker.Add(putnik);
        }
    }
}
